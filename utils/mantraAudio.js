...
export function speakMantra(text, lang = 'sanskrit', onDone, onError) {
  ...
  Speech.speak(text, {
    ...
    // A user stop/pause is not a completed recitation.
    onStopped: () => {},
  });
}
...
export async function addRecentMantra(mantra) {
  try {
    ...
    const ids = recent.map(item => typeof item === 'string' ? item : item?.id).filter(Boolean);
    const updated = [mantra.id, ...ids.filter(id => id !== mantra.id)].slice(0, 10);
...
export async function getRecentMantras() {
  try {
    const raw = await AsyncStorage.getItem(KEYS.RECENT_MANTRAS);
    if (!raw) return [];
    const ids = JSON.parse(raw).map(item => typeof item === 'string' ? item : item?.id).filter(Boolean);
    await AsyncStorage.setItem(KEYS.RECENT_MANTRAS, JSON.stringify(ids));
    return ids;
  } catch { return []; }
}
...
export function useMantraAudio(mantra, { target = 108, autoSave = true } = {}) {
  ...
  const play = useCallback(() => {
    ...
    stopSpeaking();
    const startingCount = count >= target ? 0 : count;
    if (startingCount === 0) setCount(0);
    setStatus('playing');
    setIsPlaying(true);
    ...
    playerRef.current = createMantraPlayer(
      ...
      (c) => { if (isMounted.current) { const total = startingCount + c; setCount(total); if (total >= target) { setStatus('done'); setIsPlaying(false); } } },
      () => { if (isMounted.current) { setStatus('idle'); setIsPlaying(false); } }
    );
    playerRef.current.start(target - startingCount);
  }, [mantra, count, target]);
