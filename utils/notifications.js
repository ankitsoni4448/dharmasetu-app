...
async function scheduleDaily(id, { title, body, hour, minute }) {
  if (!Notifications) return;
  try {
    // P4 FIX: Proper Expo DateTrigger — validates parameters to avoid Hermes warnings
    // DateTrigger: repeats daily at specific hour:minute
    // Strict bounds: hour 0-23, minute 0-59
    const validHour = typeof hour === 'number' 
      ? Math.max(0, Math.min(23, Math.floor(hour))) 
      : 7;
    const validMinute = typeof minute === 'number' 
      ? Math.max(0, Math.min(59, Math.floor(minute))) 
      : 0;

    // ─ Schedule with retry logic for platform stability ─
    const triggerConfig = {
    ...
    // Suppress duplicate scheduling warnings
    try {
    ...
    await Notifications.scheduleNotificationAsync({
      ...
      content: { 
        ...
        body, 
        sound: true,
        // Avoid rich media issues on older Android
        ios: { sound: true },
        android: { sound: true, channelId: 'default' },
      },
      trigger: triggerConfig,
    });
    console.log(`[Notifications] Scheduled daily: ${id} at ${validHour}:${String(validMinute).padStart(2, '0')}`);
  } catch(e) { 
    // Non-fatal — app continues without notification
    if (e.message && !e.message.includes('already scheduled')) {
      console.log('[Notifications] scheduleDaily warning:', id, e.message);
    }
  }
}
...
export async function getExpoPushToken() {
  ...
  if (process.env.EXPO_PUBLIC_REMOTE_PUSH_ENABLED !== 'true') return null;
  try {
