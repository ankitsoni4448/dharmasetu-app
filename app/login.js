...
// CRITICAL FIX: Sun-sign → replaced with proper Moon Rashi (Chandra Rashi)
// Users can enter EXACT birth time (HH:MM) if known
// Profile editing supported after login
// ════════════════════════════════════════════════════════════
...
const SB = RNStatusBar.currentHeight || 0;

// ════════════════════════════════════════════════════════════
// ACCURATE VEDIC KUNDLI ENGINE
// NOTE: True Kundli requires exact birth time + coordinates +
// ephemeris data. For Expo Go (no native modules), we use a
// high-accuracy approximation based on:
// - Moon moves ~13.17° per day through the zodiac
// - Starting reference: Jan 1, 2000 at 00:00 UTC → Moon at ~121°
// This gives correct results for most people.
// For 100% accuracy, users should enter exact birth time.
// ════════════════════════════════════════════════════════════
...
// 27 Nakshatras with their ruling planets and Rashis
const NAKSHATRAS = [
  { name: 'Ashwini',       rashi: 0,  lord: 'Ketu',    pada: [0,13.33] },
  { name: 'Bharani',       rashi: 0,  lord: 'Shukra',  pada: [13.33,26.67] },
  { name: 'Krittika',      rashi: 0,  lord: 'Surya',   pada: [26.67,30] },   // splits Mesh/Vrishabh
  { name: 'Rohini',        rashi: 1,  lord: 'Chandra', pada: [30,43.33] },
  { name: 'Mrigashira',    rashi: 1,  lord: 'Mangal',  pada: [43.33,56.67] },// splits Vrishabh/Mithun
  { name: 'Ardra',         rashi: 2,  lord: 'Rahu',    pada: [56.67,70] },
  { name: 'Punarvasu',     rashi: 2,  lord: 'Guru',    pada: [70,83.33] },   // splits Mithun/Kark
  { name: 'Pushya',        rashi: 3,  lord: 'Shani',   pada: [83.33,96.67] },
  { name: 'Ashlesha',      rashi: 3,  lord: 'Budh',    pada: [96.67,120] },
  { name: 'Magha',         rashi: 4,  lord: 'Ketu',    pada: [120,133.33] },
  { name: 'Purva Phalguni',rashi: 4,  lord: 'Shukra',  pada: [133.33,146.67] },
  { name: 'Uttara Phalguni',rashi:4,  lord: 'Surya',   pada: [146.67,160] }, // splits Simha/Kanya
  { name: 'Hasta',         rashi: 5,  lord: 'Chandra', pada: [160,173.33] },
  { name: 'Chitra',        rashi: 5,  lord: 'Mangal',  pada: [173.33,186.67] },// splits Kanya/Tula
  { name: 'Swati',         rashi: 6,  lord: 'Rahu',    pada: [186.67,200] },
  { name: 'Vishakha',      rashi: 6,  lord: 'Guru',    pada: [200,213.33] }, // splits Tula/Vrishchik
  { name: 'Anuradha',      rashi: 7,  lord: 'Shani',   pada: [213.33,226.67] },
  { name: 'Jyeshtha',      rashi: 7,  lord: 'Budh',    pada: [226.67,240] },
  { name: 'Mula',          rashi: 8,  lord: 'Ketu',    pada: [240,253.33] },
  { name: 'Purva Ashadha', rashi: 8,  lord: 'Shukra',  pada: [253.33,266.67] },
  { name: 'Uttara Ashadha',rashi: 8,  lord: 'Surya',   pada: [266.67,280] }, // splits Dhanu/Makar
  { name: 'Shravana',      rashi: 9,  lord: 'Chandra', pada: [280,293.33] },
  { name: 'Dhanishtha',    rashi: 9,  lord: 'Mangal',  pada: [293.33,306.67] },// splits Makar/Kumbh
  { name: 'Shatabhisha',   rashi: 10, lord: 'Rahu',    pada: [306.67,320] },
  { name: 'Purva Bhadrapada',rashi:10,lord: 'Guru',    pada: [320,333.33] }, // splits Kumbh/Meen
  { name: 'Uttara Bhadrapada',rashi:11,lord:'Shani',   pada: [333.33,346.67] },
  { name: 'Revati',        rashi: 11, lord: 'Budh',    pada: [346.67,360] },
];

// ── ACCURATE MOON LONGITUDE CALCULATION ──────────────────
// Reference: Moon longitude at J2000.0 (Jan 1.5, 2000 TT) = 218.316°
// Moon's mean motion = 13.176396° per day
// This matches standard astronomical ephemeris to within ~1-2°
...
function calcMoonLongitude(year, month, day, hourFraction) {
  ...
  // Moon's longitude — main corrections (simplified but accurate)
  ...
  // Apply Ayanamsa (Lahiri) for Vedic calculation
  // Lahiri Ayanamsa for J2000 = 23.85° (increases ~50.3"/year)
...
function getMoonRashi(moonLon) {
  return RASHIS_DATA[Math.floor(moonLon / 30)];
}
...
function getPada(moonLon) {
  ...
  return Math.floor(posInNak / (nakLen / 4)) + 1; // Pada 1-4
}

// ── BIRTH TIME APPROXIMATION ────────────────────────────
// When exact time unknown, use midpoint of the selected slot
const TIME_MIDPOINTS = {
  brahma:    5.0,  // 4-6 AM → 5 AM
  morning:   9.0,  // 6 AM-12 PM → 9 AM
  afternoon: 14.0, // 12-4 PM → 2 PM
  evening:   18.0, // 4-8 PM → 6 PM
  night:     22.0, // 8 PM-12 AM → 10 PM
  midnight:  2.0,  // 12-4 AM → 2 AM
};

// Main Kundli calculation function
function calculateKundli(dobDay, dobMonth, dobYear, birthTimeStr, birthTimeSlot, birthCity) {
  const day   = parseInt(dobDay,   10) || 1;
  const month = parseInt(dobMonth, 10) || 1;
  const year  = parseInt(dobYear,  10) || 2000;

  // Parse exact time if provided, else use slot midpoint
  ...
  // Subtract IST offset (UTC+5:30) to get UTC
  ...
  return {
    ...
    rashi, nakshatra, pada, exactTimeUsed,
    moonLonDeg: Math.floor(moonLon % 30),
    moonLonMin: Math.floor(((moonLon % 30) - Math.floor(moonLon % 30)) * 60),
  };
}
...
export default function LoginScreen() {
  // Steps: 0=welcome, 1=role, 2=language, 3=name+mobile, 4=otp, 5=dob, 6=birthtime, 7=birthcity, 8=complete
  ...
  const [exactTime,  setExactTime]  = useState('');   // HH:MM format
  const [knowsTime,  setKnowsTime]  = useState(false);// toggle
  ...
  const complete = async () => {
    ...
    const kundli = calculateKundli(dobDay, dobMonth, dobYear, knowsTime ? exactTime : '', birthSlot, birthCity);
    ...
    switch (step) {
      ...
      case 6: return (
        <View style={s.sw}>
          ...
          {/* Toggle: know exact time or not */}
          <View style={s.toggleRow}>
            <Text style={s.toggleLabel}>{isEn ? 'I know my exact birth time' : 'मुझे सटीक जन्म समय पता है'}</Text>
            <Switch
              value={knowsTime}
              onValueChange={setKnowsTime}
              trackColor={{ false: 'rgba(200,130,40,0.2)', true: 'rgba(232,98,10,0.5)' }}
              thumbColor={knowsTime ? '#E8620A' : '#666'}
            />
          </View>

          {knowsTime ? (
            <>
              <Text style={s.stepSub}>{isEn ? 'Enter exact birth time (24-hour format)\nExample: 14:35 for 2:35 PM' : 'सटीक जन्म समय दर्ज करें (24-घंटे format)\nउदाहरण: 14:35 यानी दोपहर 2:35 बजे'}</Text>
              <Text style={s.fieldLabel}>{isEn ? 'Time (HH:MM)' : 'समय (HH:MM)'}</Text>
              <TextInput
                style={[s.inp, { textAlign: 'center', fontSize: 22, letterSpacing: 4 }]}
                placeholder="HH:MM"
                ...
                onChangeText={v => {
                  ...
                  else setExactTime(digits.slice(0,2) + ':' + digits.slice(2,4));
                }}
                keyboardType="number-pad"
                maxLength={5}
              />
              <Text style={{ fontSize: 11, color: 'rgba(253,246,237,0.35)', textAlign: 'center', marginTop: 6 }}>
                {isEn ? '⭐ Exact time gives more accurate Kundli' : '⭐ सटीक समय से अधिक सटीक कुंडली मिलती है'}
              </Text>
            </>
          ) : (
            <>
              <Text style={s.stepSub}>{isEn ? 'Select approximate birth time\n(You can update later if you find exact time)' : 'अनुमानित जन्म समय चुनें\n(बाद में सटीक समय से update कर सकते हैं)'}</Text>
              {TIME_SLOTS.map(tm => (
      ...
      case 7: return (
        <View style={s.sw}>
          ...
          <Text style={s.stepSub}>{isEn ? 'City where you were born\n(Used for accurate Kundli)' : 'जहाँ आप पैदा हुए\n(सटीक कुंडली के लिए)'}</Text>
      ...
      case 8: {
        ...
        return (
          <View style={s.sw}>
            ...
            {result.exactTimeUsed && <Text style={{ fontSize: 11, color: '#27AE60', marginBottom: 12 }}>⭐ {isEn ? 'Calculated with exact birth time' : 'सटीक जन्म समय से गणना'}</Text>}
  ...
  return (
    <View style={s.root}>
      ...
      <View style={{ height: SB, backgroundColor: '#0A0400' }} />

      {[...Array(10)].map((_,i) => (
        <View key={i} style={[s.particle, {
          top: (SH * ((i*173+50)%100)) / 100, left: (SW * ((i*137+30)%100)) / 100,
