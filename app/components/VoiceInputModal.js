 ...
 * @param {Function} props.onClose     - called when dismissed
 */
export default function VoiceInputModal({ visible, lang = 'hindi', onSubmit, onClose }) {
  const [text, setText] = useState('');
  const slideAnim = useRef(new Animated.Value(300)).current;
  const inputRef = useRef(null);
  const isH = lang === 'hindi';
      ...
      <KeyboardAvoidingView
        ...
        <Animated.View style={[s.sheet, { transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={s.titleRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.title}>
                {isH ? 'अपना प्रश्न लिखें' : 'Type your question'}
              </Text>
              <Text style={s.sub}>
                {isH
                  ? 'DharmaChat को पूछें — शास्त्र, ज्योतिष, जीवन'
...
const s = StyleSheet.create({
  ...
  input: {
    ...
    maxHeight: 140,
    borderWidth: 1,
    borderColor: 'rgba(200,130,40,0.25)',
    lineHeight: 24,
    textAlignVertical: 'top',
  },
  charCount: {
    fontSize: 10, color: 'rgba(253,246,237,0.2)',
    textAlign: 'right', marginTop: 5, marginBottom: 12,
  },
  quickRow: {
  ...
  quickChip: {
    ...
    paddingVertical: 7,
    borderWidth: 1,
    borderColor: 'rgba(232,98,10,0.25)',
    maxWidth: '100%',
  },
  quickChipTxt: { fontSize: 11, color: '#F4A261', fontWeight: '500' },
  sendBtn: {
