...
const time = (value, timezone) => value ? new Intl.DateTimeFormat('en-IN', { timeZone: timezone || undefined, hour: 'numeric', minute: '2-digit', hour12: true }).format(new Date(value)).toUpperCase() : '—';
const periodText = (value, timezone) => value?.periods?.map(row => `${time(row.start, timezone)}–${time(row.end, timezone)}`).join(', ') || (value?.start ? `${time(value.start, timezone)}–${time(value.end, timezone)}` : '—');
const hasPeriod = value => Boolean(value?.periods?.length || (value?.start && value?.end));
