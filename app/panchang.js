...
function normalizeMonthData(value) {
  const days = safeArray(value?.days).filter(day => day && typeof day.date === 'string');
  return { ...value, days, events: safeArray(value?.events), partial: Boolean(value?.partial) || !Array.isArray(value?.days) };
}
function normalizeYearData(value) {
  ...
  const months = Array.from({ length: 12 }, (_, index) => {
    ...
    return { month, status: typeof row?.status === 'string' ? row.status : 'LOAD_MONTH_ON_DEMAND', events: safeArray(row?.events) };
  });
...
function Month({ data, onDay }) { const month = normalizeMonthData(data); return <>{month.partial ? <Text style={s.note}>Some Panchang dates could not be loaded. Only provider-returned values are shown.</Text> : <Text style={s.note}>Tap a date for full Panchang.</Text>}{!month.days.length ? <Text style={s.note}>No authoritative Panchang dates are available for this month.</Text> : <View style={s.grid}>{month.days.map(day => <TouchableOpacity key={day.date} style={[s.day, !day.available && s.dayUnavailable]} onPress={() => day.available && onDay(day.date)}><Text style={s.dayNumber}>{Number(day.date.slice(-2))}</Text><Text numberOfLines={1} style={s.dayValue}>{day.available ? day.tithi : 'Unavailable'}</Text>{safeArray(day.events).length ? <Text style={s.marker}>●</Text> : null}</TouchableOpacity>)}</View>}</>; }
function Year({ data, onMonth }) { const overview = normalizeYearData(data); return <><Text style={s.note}>{overview.partial ? 'Detailed Panchang is loaded month by month. Annual event coverage is incomplete.' : 'Months load on demand. The year view never launches 365 provider calls.'}</Text>{!overview.events.length ? <Text style={s.note}>No authoritative annual events are currently cached.</Text> : null}{overview.months.map(row => <TouchableOpacity key={row.month} style={s.month} onPress={() => onMonth(row.month)}><Text style={s.monthName}>{new Date(overview.year, row.month - 1).toLocaleDateString('en-IN', { month: 'long' })}</Text><Text style={s.monthStatus}>{row.status.replaceAll('_', ' ')}</Text>{row.events.slice(0, 3).map((event, index) => <Text key={`${event.name}-${index}`} style={s.note}>• {event.name}</Text>)}</TouchableOpacity>)}</>; }
