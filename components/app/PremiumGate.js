...
async function verifyPremiumServer(phone) {
  try {
    return isPaidPlan(await getCurrentPlan({ refresh: true }));
  } catch {
...
export default function PremiumGate({
  ...
  useEffect(() => {
    ...
    (async () => {
      const serverResult = await verifyPremiumServer();
      if (isMounted.current) setIsPremium(serverResult === true);
    })();
...
export function usePremiumGate({ serverVerify = false } = {}) {
  ...
  useEffect(() => {
    (async () => {
      const srv = await verifyPremiumServer();
      if (isMounted.current) setIsPremium(srv === true);
    })();
  }, [serverVerify]);
